    ...
    try{
        ...
        .then((result)=> {
            if(result.isConfirmed){
                ...
                fetch(`apagoespecia/cargarDeudasEspeciales/${idBusqueda}`)
                .then(respRender => respRender.json())
                .then(respuestas => {
                    datosUsuarioDetalle.innerHTML='';
                    datosUsuarioDetalle.classList.add('tabla-contenedor-medio');
                    ...
                    if(respuestas[0]==null){
                        ...
                        respuestas[0].forEach(detallePago => {
                            ...
                            columnaDetalles.innerHTML=`
                                <div class="row">
                                    <div class="col-md-2 col-6">${detallePago.CODIGO_DETA}</div>
                            ...
                            `;
                            filaDetallesPago.appendChild(columnaDetalles);
                            const columnaCheckBox = document.createElement('td');
                            const grupoCheckBox = document.createElement('div');
                            //grupoCheckBox.classList.add('custom-control','custom-checkbox');
                            ...
                            grupoCheckBox.appendChild(inputCheckboxClicker);
                            grupoCheckBox.appendChild(labelCheckbocClicker);
                            columnaCheckBox.appendChild(grupoCheckBox);
                            filaDetallesPago.appendChild(columnaCheckBox);

                            cuerpoTablaDetalles.append(filaDetallesPago);
    ...
    try {
        if(validarMetodo() && validarRecibo()){
            Swal.fire({
                ...
                cancelButtonColor: '#9C0000',
                cancelButtonText: 'No, mejor no',
                html: '¿Desea hacer el cobro de estos conceptos?',
            })
            .then((result)=>{
                if(result.isConfirmed){
                    let textMovimiento = document.querySelector('#textMovimiento');
                    ...
                    fetch('apagoespecia/realizarAjusteParcial', {
                        method: 'POST',
