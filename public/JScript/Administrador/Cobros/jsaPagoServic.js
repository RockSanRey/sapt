...
let botonMostrarHistorial = document.querySelector('#botonMostrarHistorial');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    botonMostrarHistorial.addEventListener('click', () => {
        verHistoricoPagado(botonMostrarHistorial);
    })
    ...
    try{
        ...
        buscadorUsuarioPago.innerHTML=`
            <div class="input-group mb-3">
                ...
                <select id="textMetodoBus" name="textMetodoBus" class="custom-select custom-select-lg col-md-2" autofocus></select>
                <input type="text" class="form-control form-control-lg col" name="textUsuario" id="textUsuario" placeholder="" autocomplete="off"/>
    ...
    try {
        ...
        .then(respuestas => {
            if(respuestas.estatus=='error'){
                ...
                `;
                buscadorUsuarioPago.appendChild(capaSinDatos);
                ...
                buscadorUsuarioPago.innerHTML='';
                datosUsuarioPago.appendChild(tablaContratos);
            ...
            botonResetear.addEventListener('click', () => {
                ...
                textConceptos.setAttribute('disabled','disabled');
                botonAgregarMultas.setAttribute('disabled','disabled');
                botonAgregarAnticipo.setAttribute('disabled','disabled');
                botonMostrarHistorial.setAttribute('disabled','disabled');
                buscadorUsuarios();
            })
        })
    } catch (errorAlert) {
    ...
    try {
        ...
        .then((result)=> {
            if(result.isConfirmed){
                ...
                textoContratoCobro.innerHTML=`Cobro Contrato ${seccContrato[1]}`;
                configuracionInteracciones();
                ...
                .then(respuestas => {
                    ...
                    if(respuestas[0]==null){
                        ...
                        textConceptos.removeAttribute('disabled');
                        ...
                        botonMostrarHistorial.classList.add('btn','btn-light','btn-sm');
                        botonMostrarHistorial.removeAttribute('disabled','disabled');
                        botonMostrarHistorial.setAttribute('datahistor',idBusqueda);
                        botonMostrarHistorial.setAttribute('data-toggle','modal');
                        botonMostrarHistorial.setAttribute('data-target','#formCobroDeuda');
                        botonMostrarHistorial.innerHTML='Mostrar Historial <i class="fas fa-clock"></i>';

                        respuestas[0].forEach(detallePago => {
                            const filasDetalles = document.createElement('tr');
                            filasDetalles.setAttribute('dataparent',detallePago.idTablePk);
                            ...
                            // botonEliminarServ.setAttribute('data-toggle','modal');
                            // botonEliminarServ.setAttribute('data-target','#formCobroDeuda');
                    ...
                    textConceptos.removeAttribute('disabled','disabled');
...
const configuracionInteracciones = async () => {
    try {
        let botonResetear = document.querySelector('#botonResetear');
        let botonPagoParcial = document.querySelector('#botonPagoParcial');
        let botonAgregarAnticipo = document.querySelector('#botonAgregarAnticipo');
            ...
            if(respuestas[0]==null){
                ...
                respuestas[0].forEach(detallePago => {
                    ...
                    columnaDetalles.innerHTML=`
                        <div class="row">
                            <div class="col-md-2 col-12">${detallePago.CODIGO_DETA}</div>
                            <div class="col-md-7 col-12">${detallePago.DESCRIPCION_CONC}</div>
                            <div class="col-md-1 col-2">${parseInt(detallePago.CANTIDAD_DETA).toFixed(0)}</div>
                            <div class="col-md-1 col-5">${'$'+parseFloat(detallePago.COSTO_DETA).toFixed(2)}</div>
                            <div class="col-md-1 col-5">${'$'+parseFloat(detallePago.TOTAL_DETA).toFixed(2)}</div>
                        </div>
                    `;
                    filasDetalles.appendChild(columnaDetalles);
                    const columnaAcciones = document.createElement('td');
...
const eliminarServicioCobro = async (botonEliminarServ) => {
    try{
        ...
        Swal.fire({
            ...
            input: 'textarea',
            inputAttributes: {
                minlength: 20, // Establecer un mínimo de 20 caracteres
                placeholder: 'Ingresa el motivo aquí',
            },            
            inputValidator: (value) => {
                if(!value){
                    return 'Debes ingresar un motivo para la eliminación';
                }
                if(value.lenght < 20){
                    return 'Motivo debe tener al menos 20 caracteres';
                }
            },
        })
        .then((result) => {
            if(result.isConfirmed){
                const datosEliminado = {
                    ...
                    textMotivo: result.value
                }
                let filaEliminada = document.querySelector('tr[dataparent="'+idBusqueda+'"]');
                fetch(`apagoservic/eliminarDetallesPago`, {
                    method: 'POST',
                    body: JSON.stringify(datosEliminado),
                    headers: {
                        "Content-type": "application/json",
                    },
                })
                .then(respRender => respRender.json())
                .then(respuestas => {
                    botonEliminarServ.innerHTML='<i class="fas fa-eraser"></i>';
                    if(respuestas.estatus=='error'){
                        return Swal.fire({
                            title: respuestas.title,
                            icon: respuestas.icon,
                        ...
                        filaEliminada.classList.add('bg-danger');
                        botonEliminarServ.parentNode.parentNode.remove();
                        let idBusqueda = textMovimiento.value;
                        actualizarCostoCobro(idBusqueda);
                        botonCancelar.click();
                        return Swal.fire({
                            title: respuestas.title,
                            icon: respuestas.icon,
...
const actualizarCostoCobro = async (idBusqueda) => {
    try {
        let textoDeudaTotal = document.querySelector('#textoDeudaTotal');
        fetch(`apagoservic/calculoTotalDeuda/${idBusqueda}`)
        .then(respRender => respRender.json())
        .then(respuestas => {
            ...
            if(respuestas==null){
                ...
                respuestas.forEach(totales => {
                    ...
                    textMetodo.innerHTML=`
                        ...
                        <option value="EFEC" selected="selected">Efectivo</option>
                ...
                cabeceraComprobante.innerHTML=`
                    ...
                    <tr>
                        ...
                        <td colspan="2" style="border: 1px solid rgb(20,179,237)">
                            ...
                            <div style="text-align: center; margin-right: 5px; font-size:10px;">${usuarios.CODBARR_CLIEN}</div>
                        </td>
                        <td colspan="3" style="border: 1px solid rgb(20,179,237)">
...
function validarMotivo(){
    ...
    if(inputForm.value==null || inputForm.value==''){
        ...
        return false;
    }else if(inputForm.value.length<20){
        Swal.fire({
            title: 'Campo Inválido',
            confirmButtonText: 'Revisar',
            confirmButtonColor: '#9C0000',
            icon: 'error',
            text: 'Motivo minimo 20 carácteres.',
        }).then((result)=>{
            if(result.isConfirmed){
                inputError(inputForm);
            }
