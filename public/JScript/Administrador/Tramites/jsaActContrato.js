...
document.addEventListener('DOMContentLoaded', () => {
    ...
    botonCancelar.addEventListener('click', () => {
        let datosBajasTemplate = document.querySelector('#datosBajasTemplate');
...
const plantillaBusqueda = async () => {
    try{
        let busquedaBajas = document.querySelector('#busquedaBajas');
        busquedaBajas.innerHTML=`
...
const completarBusquedaBajas = async (textContrato) => {
    try{
        ...
        if(textContrato.value=='' || textContrato.value==null){
            ...
            fetch(`aactcontrato/autoCompletarBajasTem/${idBusqueda}`)
        ...
        .then(respuestas => {
            ...
            if(respuestas.estatus=='error'){
                ...
                respuestas.forEach(bajas => {
                    ...
                    const botonReactivarBaja = document.createElement('button');
                    botonReactivarBaja.classList.add('btn','btn-info','btn-sm');
                    botonReactivarBaja.setAttribute('data-toggle','modal');
                    botonReactivarBaja.setAttribute('data-target','#formRegistroDatos');
                    botonReactivarBaja.setAttribute('dataedit',bajas.idTablePk);
                    botonReactivarBaja.setAttribute('id','botonEditarSel');
                    botonReactivarBaja.innerHTML = '<i class="fa fa-edit"></i>';
                    botonReactivarBaja.addEventListener('click',() => {
                        editarBajaContrato(botonReactivarBaja);
                    });
                    const botonImprimirReactiva = document.createElement('button');
                    botonImprimirReactiva.classList.add('btn','btn-success','btn-sm','d-none');
                    ...
                    grupoAcciones.appendChild(botonReactivarBaja);
...
const editarBajaContrato = async (botonReactivarBaja) => {
    try {
        let datosBajasTemplate = document.querySelector('#datosBajasTemplate');
        let labelTitleModal = document.querySelector('#labelTitleModal');
        let idBusqueda = botonReactivarBaja.attributes.dataedit.value;
        labelTitleModal.innerHTML='Reactivar Baja';
        datosBajasTemplate.innerHTML=cargaAnimacion;
        fetch(`aactcontrato/buscarEditarBajas/${idBusqueda}`)
        .then(respRender => respRender.json())
        .then(respuestas => {
            respuestas.forEach(bajas => {
                datosBajasTemplate.innerHTML=`
                    <div class="bg-white p-2">
                        <div class="form-group mb-2">
                            <input type="hidden" id="textIdContrato" name="textIdContrato="textIdContrato" value="${bajas.IDCONTRATO}">
                            <div class="form-control form-control-sm">${bajas.CONTRATO_CBAJA} - ${bajas.CLIENTE}</div>
                        </div>
                        <div class="row">
                            <div class="form-group mb-2 col-sm-6 col-12">
                                <div class="form-control form-control-sm">${bajas.DIRECCION}</div>
                            </div>
                            <div class="form-group mb-2 col-sm-6 col-12">
                                <div class="form-control form-control-sm"><small>Tramita Baja:</small> ${bajas.COMITE}</div>
                            </div>
                        </div>
                        <div class="form-group mb-2">
                            <textarea class="form-control form-control-sm" readonly>${bajas.MOTIVBAJA_CBAJA}</textarea>
                        </div>
                        <div class="form-group mb-2">
                            <select class="custom-select custom-select-sm col-sm-6" id="textEstatus" name="textEstatus"></select>
                        </div>
                        <div class="form-group mb-2">
                            <textarea class="form-control form-control-sm" maxlength="100" id="textMotivo" name="textMotivo"rows="5" placeholder="Motivo de la reactivación"></textarea>
                        </div>
                    </div>
                `;
                estatusSelec=bajas.ESTATUS_CCONT;
                llenarComboContratosEstatus(estatusSelec);
            })
        })
    } catch (errorAlert) {
        return Swal.fire({
...
const llenarComboContratosEstatus = async (estatusSelec) => {
    try {
        let textEstatus = document.querySelector('#textEstatus');
        textEstatus.innerHTML='<option>Estatus Contrato</option>';
...
const reactivarContrato = async () => {
    try {
        if(validarContrato() && validarEstatus() && validarMotivo()){
            Swal.fire({
                title: 'Reactivacion',
                icon: 'question',
                ...
                html: '¿De verdad desea reactivar este contrato?',
            })
            .then((result)=>{
                if(result.isConfirmed){
                    ...
                    let botonImprimirSel = document.querySelector('#botonImprimirSel');
                    const reactivaDatos = {
                        textIdContrato: textIdContrato.value,
                        textEstatus: textEstatus.value,
                        textMotivo: textMotivo.value,
                    };
                    fetch('aactcontrato/reactivarContratoBaja', {
                        method: 'POST',
                        body: JSON.stringify(reactivaDatos),
                        headers: {
                        ...
                        if(respuestas.estatus=='error'){
                            ...
                            return Swal.fire({
                                ...
                                if((result.isDismissed)){
                                    botonImprimirSel.classList.remove('d-none');
                                    botonCancelar.click();
                                }
                            })
                        }
...
function validarEstatus(){
    ...
    if(inputForm.value==null || inputForm.value==''){
        ...
        return false;
    }else if(inputForm.value=='BAJA' || inputForm.value=='INAC'){
        Swal.fire({
            title: 'Dato Inválido',
            confirmButtonText: 'Revisar',
            confirmButtonColor: '#9C0000',
            icon: 'error',
            text: 'Se supone que vas a reactivar el contrato no cambia de estatus.',
        }).then((result)=>{
            if(result.isConfirmed){
                inputError(inputForm);
            }
